...
export default function ProjectPage() {
    ...
    const autoSummarize = async (filename: string) => {
        ...
        try {
            const searchRes = await client.post(
                ...
                {
                    query: `Provide a comprehensive summary of "${filename}". What are the main topics, key points, and important information it contains?`,
                    n_results: 5,
                    use_hybrid: true
                }
            )
            setMessages(prev => [...prev, {
                ...
                role: 'assistant',
                content: `📄 **${filename}** uploaded!\n\n**Here's what's inside:**\n\n${searchRes.data?.answer || 'Document processed. You can now ask questions about it.'}`,
                sources: searchRes.data?.sources,
                tokens: searchRes.data?.tokens_used,
    ...
    const handleSend = async () => {
        ...
        try {
            const searchRes = await client.post(
                `/api/v1/knowledge/${id}/search`,
                { query: currentInput, n_results: 5, use_hybrid: true }
            )
            if (searchRes.data?.answer && searchRes.data.chunks_used > 0) {
                setMessages(prev => [...prev, {
                    id: (Date.now() + 1).toString(),
                    role: 'assistant',
                    content: searchRes.data.answer,
                    sources: searchRes.data.sources,
                    tokens: searchRes.data.tokens_used,
                    agents: ['ResearchAgent']
                }])
            } else {
                const result = await runAgent(id!, currentInput)
                setMessages(prev => [...prev, {
                    id: (Date.now() + 1).toString(),
                    role: 'assistant',
                    content: result.final_output || 'No answer generated',
            ...
            try {
                ...
                setMessages(prev => [...prev, {
                    id: (Date.now() + 1).toString(),
                    role: 'assistant',
                    content: `Error: ${agentErr.response?.data?.detail || 'Something went wrong'}`
                }])
            }
        } finally {
            setLoading(false)
    ...
    return (
        <div className="flex min-h-screen" style={{ background: '#0A0A0F' }}>
            ...
            <div className="ml-64 w-72 flex flex-col border-r" style={{ borderColor: 'rgba(99,102,241,0.1)', background: '#0A0A0F' }}>
                ...
                {/* Document list with source selector */}
                ...
                <div className="px-6 py-4 border-b flex items-center gap-3"
                    ...
                    <div>
                        ...
                        <p className="text-gray-600 text-xs">
                            {selectedDocs.length > 0
                                ? `🎯 Searching ${selectedDocs.length} selected source${selectedDocs.length > 1 ? 's' : ''} only`
                                : 'Searching all documents · DecisionAgent · ResearchAgent · CriticAgent'}
                        </p>
                    </div>
                </div>
                    ...
                    {messages.length === 0 && (
                        <div className="flex flex-col items-center justify-center h-full text-center">
                            ...
                            <p className="text-gray-600 text-sm max-w-sm">
                                Upload a document or image on the left — I'll automatically summarize it so you know what's inside before asking questions.
                            </p>
                    ...
                    {messages.map((msg) => (
                        <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                            <div className="max-w-xl w-full">
                                ...
                                {msg.role === 'assistant' && (msg.agents || msg.tokens || msg.sources) && (
                    ...
                    <div className="flex items-end gap-3 p-3 rounded-2xl"
                        ...
                        <textarea
                            ...
                            placeholder={selectedDocs.length > 0
                                ? `Ask about ${getSelectedDocNames()}...`
                                : 'Ask anything about your documents and images...'}
                            rows={1}
